...
def mypage_view():
    ...
    st.caption(
        "면접관에게 어필할 객관적인 '팩트'만 입력해 주세요. 부족한 스토리는 AI가 대화로 끌어내 줍니다."
    )
    ...
    with st.form("resume_form"):
        ...
        with tab1:
            ...
            with col1:
                eng_name = st.text_input(
                    "영문 이름", value=personal.get("eng_name", "")
                )
            with col2:
                gender_opts = ["선택안함", "남성", "여성"]
                curr_gender = personal.get("gender", "선택안함")
                gender_idx = (
                    gender_opts.index(curr_gender) if curr_gender in gender_opts else 0
                )
        ...
        with tab3:
            ...
            awards = st.text_area(
                "수상 내역 및 대외활동", value=add.get("awards", ""), height=150
            )
            tech = st.text_input(
                "기술 스택 / 자격증 (쉼표로 구분)", value=add.get("tech_stack", "")
            )
